...
{
    return preg_split('/\\r\\n|\\r|\\n/', run('dir /S /B * | findstr /v /i .git\ | findstr /v /i vendor | findstr /v /i ' . basename(__FILE__) . ' | findstr /r /i /M /F:/ ":author :vendor :package VendorName skeleton vendor_name vendor_slug author@domain.com :coverage_value :default_branch :codecov_token"'));
}

function replaceForAllOtherOSes(): array
{
    return explode(PHP_EOL, run('grep -E -r -l -i "<\w+-->|\# <\w+> \#|\#<\w+>\#|\/\*\* <\w+> \*\/|\/\*\*<\w+>\*\/|:author|:vendor|:package|VendorName|skeleton|vendor_name|vendor_slug|author@domain.com|:coverage_value|:default_branch|:codecov_token" --exclude-dir=vendor ./* ./.php-cs-fixer.dist.php ./.github/* | grep -v ' . basename(__FILE__)));
}
